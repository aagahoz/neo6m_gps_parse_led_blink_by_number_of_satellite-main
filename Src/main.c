...
//	printf("%s\n\n\n\n", p);
	...
	counter = 0;
	virgul_sayisi = 0;
	memset(obje->section, 0, 255);
	while (p[counter] != '\n' && counter < 255)
	{
		if (p[counter] == ',')
			virgul_sayisi++;

		obje->section[counter] = p[counter];
...
/* Configure the peripherals common clocks */
  ...
  HAL_UART_Receive_IT(&hlpuart1, (uint8_t *)raw_array, 256);
...
//	HAL_UART_Receive(&hlpuart1, (uint8_t *) raw_array, 256, 2000);
	  HAL_UART_Receive_IT(&hlpuart1, (uint8_t *)raw_array, 256);

	gps_parser(raw_array, strlen(raw_array), &DATALAR);


    HAL_UART_Transmit(&huart1, (uint8_t *)raw_array, 256, 0xFFFF);


    if (DATALAR.numberOfSatellites == 8)
    {
